	...
	n.oPagination = {
		two_button: {
			fnInit: function(g, m, r) {
				...
				m.appendChild(s);
				m.appendChild(w);
				j(s).bind("click.DT", function() { g.oApi._fnPageChange(g, "previous") && r(g); });
				j(w).bind("click.DT", function() { g.oApi._fnPageChange(g, "next") && r(g); });
				j(s).bind("selectstart.DT", function() { return false; });
				j(w).bind("selectstart.DT", function() { return false; });
		...
		full_numbers: {
			fnInit: function(g,
				...
				m.appendChild(F);
				m.appendChild(x);
				j(s).bind("click.DT", function() { g.oApi._fnPageChange(g, "first") && r(g); });
				j(w).bind("click.DT", function() { g.oApi._fnPageChange(g, "previous") && r(g); });
				j(F).bind("click.DT", function() { g.oApi._fnPageChange(g, "next") && r(g); });
				j(x).bind("click.DT", function() { g.oApi._fnPageChange(g, "last") && r(g); });
				j("span", m).bind("mousedown.DT", function() { return false; }).bind("selectstart.DT", function() { return false; });
	...
	j.fn.dataTable = function(g) {
		...
		function F(a, b) {
			...
			if (typeof a.aoPreSearchCols[b] ==
				"undefined" || a.aoPreSearchCols[b] === null) a.aoPreSearchCols[b] = { sSearch: "", bRegex: false, bSmart: true };
			else {
		...
		function ta(a) {
			if (a.bAjaxDataGet) {
				...
				a.fnServerData.call(a.oInstance, a.sAjaxSource, c, function(i) { ua(a, i); });
				return false;
			} else return true;
		}
...
f, e, i, h, k, l, q, t = 0; t < d.length; t++) {
		...
		function xa(a) {
			...
			c.bind("keypress.DT", function(d) { if (d.keyCode == 13) return false; });
			return b;
		}
			...
			if (typeof a != "object") {
				...
				for (c = 0; c < a.length; c++) {
					...
					if (i !== null) e.aoData[b].nTr.getElementsByTagName("td")[i].innerHTML = h;
						...
						e.aoData[b]._anHidden[c].innerHTML =
							h;
				}
			}
		...
		return this.each(function() {
			...
			if (this.nodeName.toLowerCase() != "table") H(e, 0, "Attempted to initialise DataTables on a node which is not a table: " + this.nodeName);
				...
				if (h) for (a = 0; a < g.aaData.length; a++) u(e, g.aaData[a]);
				else z(e);
